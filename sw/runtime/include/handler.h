...
static inline int spin_rdma_put(uint32_t dest, void *data, uint32_t length, uint32_t sput_len, uint32_t roffset, uint32_t stream_cmd, spin_cmd_t *handle)
{
    ...
    asm volatile(" sw      %2, 144(%1);  \
                   ...
                   sw      %9, 140(%1);  \
                   lw      %0, 128(%1);  \
    " : "=r"(res) : "r"(base_addr), "r"(dest), "r"(fid), "r"(src_addr_high), "r"((uint32_t)data), "r"(length), "r"(sput_len), "r"(roffset), "r"(cmd_info));       
