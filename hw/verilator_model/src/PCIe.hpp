...
namespace PsPIN
{
    ...
    template <typename AXIMstPortType, typename AXISlvPortType>
    class PCIe : public SimModule
    {
    ...
    private:
        AXIMaster<AXIMstPortType> axi_driver_mst;
    ...
    public:
        PCIe(AXIMstPortType &axi_mst, AXISlvPortType &axi_slv, uint32_t aw_buffer_size, uint32_t w_buffer_size, uint32_t ar_buffer_size, uint32_t r_buffer_size, uint32_t b_buffer_size, uint32_t pcie_L, uint32_t pcie_G)
        : axi_driver_mst(axi_mst), axi_driver_slv(axi_slv), pcie_L(pcie_L), pcie_G(pcie_G)
        {
            write_wait_cycles = 0;
            read_wait_cycles = 0;
    ...
    private: 
        ...
        void progress_in_flight_writes()
        {
            ...
            if (sim_time() >= write.time && axi_driver_slv.can_send_b_beat())
            {
                ...
                SIM_PRINT("PCIe got data (data size: %d)!\n", write.req.data_size);
                if (slv_write_cb) slv_write_cb(write.req.addr, write.req.w_beat.w_data, write.req.data_size);
